...
def check_duplicates(table, data_dict):
    ...
    response = supabase.table(table).select("*").eq("email_id", data_dict["email_id"]).execute()
    if len(response.data) > 0:
        return False, "This email id is already registered."
    
    response = supabase.table(table).select("*").eq("mobile_number", data_dict["mobile_number"]).execute()
    if len(response.data) > 0:
...
def create_teacher_account(data_dict):
    try:
        data = {
            ...
            "password": bcrypt.hashpw(data_dict["password"].encode("utf-8"), bcrypt.gensalt()).decode()  # applying hashing on password
        }
        try:
        ...
        return True, f"Success! Your teacher account has been created.Teacher ID is {data_dict['id']}. Please Login now. 🔐"
    except Exception as e:
        return False, str(e)
...
def teacher_login(t_id, t_password):
    try:
        ...
        return True, f"Welcome back, {data['name']}! Your dedication to teaching makes a world of difference.", data
    except Exception as e:
...
def create_student_account(data_dict):
    try:
        ...
        return True, f"Success! Your student account has been created.Student ID is {data_dict['id']}. Please Login now. 🔐"
    except Exception as e:
        return False, str(e)
...
def get_subject_id(subject_code):
    try:
        response = supabase.table('subjects').select("*").eq("code", subject_code).execute()
        if response.data:
...
def enroll_in_subject(student_id, subject_id):
    try:
        ...
        if len(response.data) > 0:
            ...
            response = supabase.table('subject_students').insert({
